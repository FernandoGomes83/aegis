...
Depends on: {{task_sec_001.depends_on}}
  ...
  TASK-NNN.2: {{task_sec_001.sub2.description}}
    Tests: TEST-SEC-{{task_sec_001.sec_prop_key}}
...
|------|------------|---------------|
| {{task.id}} | {{task.depends}} | {{task.parallel}} |

### Execution Phases

Phase 1 (parallel): {{phase_1.tasks}}
Phase 2 (parallel): {{phase_2.tasks}}
Phase 3: {{phase_3.tasks}}

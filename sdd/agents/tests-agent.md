...
name: tests-agent
description: >
  Generates tests.md and RED test files from parsed SDD artifacts.
  Dispatched by the /sdd tests command. Receives the full parsed context
...
You are the SDD tests agent. You are dispatched by the `/sdd tests` command
...
You receive the following context from `/sdd tests`:
...
> before declaring it production-ready. See sdd/framework/security/SECURITY_UNIVERSAL.md.
```
...
> declared production-ready. See sdd/framework/security/SECURITY_UNIVERSAL.md §14.
```
...
in `sdd/framework/validation/rules.yaml` (section `after_tests`) and report
...
equivalent, or from `sdd.config.yaml` if a `stack.language` field is present.
Use this to select:
...
Return the following structured summary to the `/sdd tests` command:

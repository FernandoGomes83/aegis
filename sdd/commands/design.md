...
1. Read `sdd.config.yaml` from the project root. If it does not exist, stop and tell the user to run `/sdd init` first.
2. Read `sdd/requirements.md` (or the path under `output.dir` in config). If it does not exist, stop and tell the user to run `/sdd requirements` first.
...
Read `sdd.config.yaml` and extract:

- `project.name` — used in the artifact header
- `project.language` — select i18n label set from `sdd/framework/i18n/`
...
Read `sdd/framework/security/security-properties.yaml`.
...
For each open decision below, if the answer is not determinable from `sdd.config.yaml` and is relevant to the project's requirements, ask ONE question and wait for the answer before asking the next.
...
Dispatch to `sdd/agents/design-agent.md` with the following inputs:

- **requirements_content**: full text of `requirements.md`
- **stack_config**: the stack section from `sdd.config.yaml` plus any answers from Step 4
- **user_decisions**: key/value map of answers collected in Step 4
- **template**: content of `sdd/framework/templates/design/<formalism>.template.md`
- **i18n**: loaded label set from Step 1
- **security_properties**: filtered SEC-PROP entries from Step 2
- **level_rules**: content of `sdd/framework/levels/<formalism>.md`
...
The output file path is `<output.dir>/design.md` (default: `sdd/design.md`).

---

## Step 6 — Light Validation (after_design)

After `design.md` is written, run the following checks from `sdd/framework/validation/rules.yaml`:
...
- Ask the user: "Validation found gaps. Would you like to backtrack to `/sdd requirements` to address them, or proceed with the current design.md?"
...
Next: run /sdd tasks to generate the implementation plan.
```
...
- **Do not invent stack choices.** If a decision cannot be answered from config or user input, leave the field as a placeholder (e.g., `TBD — confirm with /sdd update`) rather than guessing.
...
- **Overwrite by default.** If `design.md` already exists, overwrite it. If it contains a `NEEDS REVIEW` notice from `/sdd update`, remove the notice as part of regeneration.

...
# `/sdd init` — Initialize SDD Framework

You are executing the `/sdd init` command. Your job is to gather all project context interactively and write a valid `sdd.config.yaml` at the project root. Follow every step in order. Do not skip steps. Do not combine steps into a single prompt.
...
For each document found in Step 1, read its first 200 lines and match its content against the keyword signals defined in `sdd/framework/inputs/recommended-types.md`.
...
Using all information collected in Steps 1–6, write `sdd.config.yaml` at the project root.

The file must conform exactly to this schema:

```yaml
# sdd.config.yaml — generated by /sdd init on <ISO date>
version: "1"
...
```bash
mkdir -p docs/sdd/reports sdd/tests
```
...
Files written:
  sdd.config.yaml
  docs/sdd/reports/   (created)
  sdd/tests/          (created)

Next step: run /sdd requirements to generate requirements.md from your input documents.
```

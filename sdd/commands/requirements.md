...
1. `sdd.config.yaml` exists at the project root. If it does not, stop immediately and tell the user to run `/sdd init` first.
2. The `inputs` list in `sdd.config.yaml` contains at least one entry. If the list is empty or absent, warn the user that requirements will contain security content only, and ask whether they want to continue or add input docs first.

---

## Step 1: Load Config

Read `sdd.config.yaml`. Extract and hold in working memory:

- `project.name` — used in the artifact header.
- `project.language` — determines which i18n file to load (`en` → `sdd/framework/i18n/en.yaml`, `pt-BR` → `sdd/framework/i18n/pt-br.yaml`). Default to `en` if not set.
...
Load the level rules file for the configured formalism level from `sdd/framework/levels/<level>.md`. Note which structural elements are required vs. optional at this level.

---

## Step 2: Read All Input Docs

Read every file listed under `inputs` in `sdd.config.yaml`. For `type: auto` entries pointing to a directory, read all files found in that directory.
...
Also identify which security-relevant project characteristics are present in the input docs. Map them to the `applies_when` values used in `sdd/framework/security/security-requirements.yaml`:
...
Read `sdd/framework/security/security-requirements.yaml`.
...
If `sdd.config.yaml` has `security.extra_requirements` entries, append them to the set. They are additive only.
...
Dispatch to `sdd/agents/requirements-agent.md` with the following context package:
...
- **template** — the requirements template for the configured level from `sdd/framework/templates/requirements/<level>.template.md`.
...
- **project_name** — from `sdd.config.yaml`.
...
- Apply the formalism level rules: use the requirements format (user stories, acceptance criteria depth, glossary) specified in `sdd/framework/levels/<level>.md`.
...
Write the output to `sdd/requirements.md` (relative to the project root, using the `output.dir` from `sdd.config.yaml`, defaulting to `sdd/`).

---

## Step 7: Light Validation

After the agent writes `sdd/requirements.md`, run the `after_requirements` checks defined in `sdd/framework/validation/rules.yaml`.

Execute each check in order:

- **VAL-REQ-01** (`every_input_doc_has_derived_req`) — verify that each input document listed in `sdd.config.yaml` has at least one REQ-NNN or SEC-REQ-* entry with a matching `Derives from:` citation. Severity: warning.
...
Output written to: sdd/requirements.md
```

Then suggest the next step:

> Requirements are ready. When you're ready to define how the system will satisfy them, run `/sdd design`.

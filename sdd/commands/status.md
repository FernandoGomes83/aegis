...
# `/sdd status` — Show SDD Project State

You are executing the `/sdd status` command. Your job is to read the project configuration and all available SDD artifacts, compute a lightweight coverage snapshot, and display a concise status report. This command is **read-only** — it does not generate, modify, or delete any file.
...
Look for `sdd.config.yaml` at the project root.
...
- If the file exists, read it fully and extract:
  ...
  - `output.dir` (default: `sdd/` if not set)
...
For each file that **exists**, record its last modified timestamp (from the filesystem — use the Bash tool with `stat -f "%Sm" -t "%Y-%m-%d"` on macOS, or `stat -c "%y" | cut -d' ' -f1` on Linux).

---

## Step 3: Extract Stats from Each Artifact

For each artifact file that exists, parse its content and collect the following stats. All counts are done by scanning for the canonical ID patterns defined in `sdd/framework/SPEC.md §4`.
...
This step only runs if **two or more** artifacts exist. It computes a lightweight coverage percentage without doing full cross-artifact validation (that is reserved for `/sdd validate`).
...
An artifact is **stale** if its upstream source was modified **after** the artifact itself was last modified. Apply the propagation rules from `sdd/framework/SPEC.md §9`:
...
Also check if the artifact contains a `> NEEDS REVIEW` notice at the top (from `/sdd update`). If present, mark it as stale regardless of timestamps.
...
|-------------------------------------------------|-------------------------------------------------------------|
| No artifacts exist                              | `/sdd requirements`                                         |
| Only requirements.md exists                     | `/sdd design`                                               |
| requirements.md + design.md exist              | `/sdd tasks` and/or `/sdd tests`                            |
| requirements.md + design.md + tasks.md exist   | `/sdd tests`                                                |
| All 4 artifacts exist, none stale              | `/sdd validate` or start implementation                     |
| Any artifact is stale                           | `/sdd update [stale artifact name]` to propagate changes    |
| Coverage < 60%                                  | `/sdd validate` to find and fix coverage gaps               |
...
- Do not validate artifact content beyond the pattern-matching described in Step 3. Deep validation is reserved for `/sdd validate`.
- If a file read fails (e.g., the file is unreadable), treat it as not existing and mark it `❌`.
- The counts in Step 3 are approximate — they rely on ID pattern matching, not full parsing. This is intentional: `/sdd status` must be fast. Exact counts are the domain of `/sdd validate`.

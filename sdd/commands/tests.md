...
project root (or the `output.dir` configured in `sdd.config.yaml`):

| File | Purpose |
|------|---------|
| `sdd.config.yaml` | Framework configuration — stack, test_framework, property_testing |
...
Tell the user which SDD command to run first:

- Missing `sdd.config.yaml` → run `/sdd init`
- Missing `requirements.md` → run `/sdd requirements`
- Missing `design.md` → run `/sdd design`
- Missing `tasks.md` → run `/sdd tasks`
...
### From `sdd.config.yaml`

- `project.name` — used in the tests.md header
- `project.language` — select the i18n label set from `sdd/framework/i18n/`
...
- `output.dir` — directory where tests.md and test files are written (default: `sdd/`)
...
Dispatch to `sdd/agents/tests-agent.md` with the full parsed context from
...
- Light → `sdd/framework/templates/tests/light.template.md`
- Standard → `sdd/framework/templates/tests/standard.template.md`
- Formal → `sdd/framework/templates/tests/formal.template.md`
...
All tests are RED. Start implementation with `/sdd tasks` to see the
ordered task list, or begin implementing TASK-001.

Run `/sdd validate` at any time for a full cross-artifact coverage report.
```
...
| Condition | Action |
|-----------|--------|
| `sdd.config.yaml` is missing | Stop. Tell user to run `/sdd init` first. |
| `requirements.md` is missing | Stop. Tell user to run `/sdd requirements` first. |
| `design.md` is missing | Stop. Tell user to run `/sdd design` first. |
| `tasks.md` is missing | Stop. Tell user to run `/sdd tasks` first. |
...
| No SEC-PROP-* entries in design.md | Critical error — security properties are mandatory. Tell user to run `/sdd design` again. |

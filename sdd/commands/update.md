...
# `/sdd update [artifact]` — Update Artifact and Check Downstream Impact

You are executing the `/sdd update` command. Your job is to re-generate a chosen artifact after its sources have changed, preserve user customizations where possible, and show the downstream impact on dependent artifacts — offering to cascade updates. Follow every step in order. Do not skip steps. Do not combine steps into a single prompt.
...
/sdd update design         — re-read requirements.md, regenerate design.md
/sdd update tasks          — re-read design.md + requirements.md, regenerate tasks.md
/sdd update tests          — re-read all upstream artifacts, regenerate tests.md
```
...
Before anything else, read `sdd.config.yaml` at the project root. If it does not exist, stop immediately and tell the user:

```
sdd.config.yaml not found. Run /sdd init first.
```
...
Read the existing artifact file from the output directory declared in `sdd.config.yaml` (default: `sdd/`).

If the artifact file does not exist, tell the user:

```
<artifact>.md not found in <output.dir>.
Nothing to update — run /sdd <command> first to generate it.
```
...
|---|---|
| requirements.md | All input documents listed in `sdd.config.yaml` under `inputs` |
| design.md | `sdd/requirements.md` |
| tasks.md | `sdd/design.md` and `sdd/requirements.md` |
| tests.md | `sdd/design.md` and `sdd/requirements.md` |
...
- New SEC-REQ-* that are now applicable based on `security_features` in `sdd.config.yaml` but are absent from requirements.md
...
Run /sdd validate for a full cross-artifact audit.
```
...
Pre-filling from existing artifact:
  ...
  - Stack context: Next.js + PostgreSQL (from sdd.config.yaml)
...
(not generated by /sdd). They will be preserved in the updated artifact:
...
**Security.** Always re-inject security content at full rigor, regardless of what was in the previous artifact. Read `sdd/framework/security/SECURITY_UNIVERSAL.md` and the relevant YAML files. Do not preserve old SEC-REQ-* or SEC-PROP-* verbatim — they are replaced with the current output of the security agent.
...
```
Written: sdd/requirements.md
```

**Run light validation.** After writing, run the same light validation checks that the original generation command would run (as defined in `sdd/framework/SPEC.md §6`). Report the results inline:
...
Run /sdd update <artifact> when you're ready to propagate changes.
```
...
Update cascade complete.
  ...
  Next step: run /sdd update tests or /sdd tests to complete propagation.
```

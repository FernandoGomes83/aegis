...
Full cross-artifact validation. Produces a coverage matrix, security audit, gaps report, and stats. Writes the complete report to `docs/sdd/reports/validation-YYYY-MM-DD.md` and prints a summary to the terminal.

---

## Prerequisites

1. Read `sdd.config.yaml`. If it does not exist, stop and tell the user to run `/sdd init` first.
...
| File | Artifact |
|------|----------|
| `sdd/requirements.md` (or the path from `output.dir` in config) | Requirements |
| `sdd/design.md` | Design |
| `sdd/tasks.md` | Tasks |
| `sdd/tests.md` | Tests |

Also read:
- `sdd/framework/security/security-requirements.yaml` — canonical SEC-REQ-* catalog
- `sdd/framework/security/security-properties.yaml` — canonical SEC-PROP-* catalog
- `sdd/framework/security/SECURITY_UNIVERSAL.md` — for §14 checklist items
- `sdd/framework/validation/rules.yaml` — validation rule definitions
...
> Warning: Only N artifact(s) found. Validation will be partial. Run `/sdd requirements`, `/sdd design`, `/sdd tasks`, and `/sdd tests` to generate missing artifacts before running a full validation.
...
5. **Status** — Apply the rules from `sdd/framework/validation/coverage-matrix.md §3`:
...
Read the §14 checklist from `sdd/framework/security/SECURITY_UNIVERSAL.md`. The checklist covers seven categories: Input e dados, Autorização, Proteção, Dados, Upload, URLs e recursos externos, and Infraestrutura.
...
4. Assign status:
   ...
   - **FAIL**: The item is applicable to this project (based on `sdd.config.yaml` features and `security.not_applicable` declarations) and at least one of SEC-REQ, SEC-PROP, or TEST-SEC is missing. A FAIL in a mandatory category (Input e dados, Autorização, Proteção, Dados, Infraestrutura) is a **blocking error**.
   - **N/A**: The item's category is listed under `security.not_applicable` in `sdd.config.yaml`. N/A cannot be auto-assigned without an explicit config declaration.
...
Run all light validation checks from `sdd/framework/validation/rules.yaml` against the present artifacts:
...
Dispatch to `sdd/agents/validation-agent.md` with all data collected in Steps 1–5. The agent writes the full validation report.

**Report file path:**

```
docs/sdd/reports/validation-YYYY-MM-DD.md
```

Where `YYYY-MM-DD` is today's date. If the file already exists, append a counter suffix:

```
docs/sdd/reports/validation-2026-04-04.md       ← first run of the day
docs/sdd/reports/validation-2026-04-04-2.md     ← second run of the same day
docs/sdd/reports/validation-2026-04-04-3.md     ← third run, etc.
```

Create the `docs/sdd/reports/` directory if it does not exist.
...
```
SDD Validation Report — <project name> — <date>
================================================
...
Overall coverage % : N%
Report written to  : docs/sdd/reports/validation-YYYY-MM-DD.md
```

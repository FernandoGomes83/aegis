# SDD Framework Specification

> Version 1.0 — 2026-04-04
> This document is the authoritative reference for the Software Design Documents (SDD) framework.
...
The SDD framework is a structured, artifact-based approach to software design documentation. It exists to close the gap between product intent and implementation by making design decisions explicit, traceable, and auditable — before and during development.
...
- **Layer 2 — Skill**: Claude Code automation. A `/sdd` skill that reads project inputs, applies the framework rules, and generates all four artifacts with full traceability.
...
The framework ships three security files under `sdd/framework/security/`:
...
Triggered explicitly via `/sdd validate`. Runs all light checks plus four additional reports. May take longer as it performs cross-artifact analysis.
...
| auto           | (any file in sdd/inputs/) | Automatically detected and classified by the requirements agent          |
...
- They must be listed in `sdd.config.yaml` under the `inputs` key, or placed in `sdd/inputs/` for auto-detection.
...
The framework ships starter templates for common input document types in `sdd/framework/inputs/templates/`. Projects that do not have an existing spec can use these templates as a starting point.

---

## 8. Configuration

Projects configure the SDD framework via a `sdd.config.yaml` file at the project root.

### File Location

```
<project-root>/
  sdd.config.yaml   <-- framework reads this
  sdd/              <-- generated artifacts live here
    requirements.md
    design.md
    tasks.md
    tests.md
...
inputs:
  ...
  - path: docs/SECURITY_UNIVERSAL.md
    type: security
  # "auto" entries: any file in sdd/inputs/ is auto-detected
  - path: sdd/inputs/
    type: auto

output:
  dir: sdd/                   # where artifacts are written (default: sdd/)
...
The SDD framework uses a manual propagation model. When a source artifact changes, downstream artifacts are marked as "needs review" rather than automatically regenerated. This prevents silent overwrites of hand-edited content.
...
When the `/sdd update` command detects that an artifact has changed, it writes a review notice to the top of each downstream artifact:

```
> NEEDS REVIEW — requirements.md was updated on 2026-04-04.
> Re-run `/sdd design`, `/sdd tasks`, and `/sdd tests` to propagate changes,
...
- Changes to requirements do not automatically regenerate design. The designer must review the change, decide whether existing properties still hold, and either re-run `/sdd design` or manually update `design.md`.
...
**Scenario 1 — New requirement added**: A new REQ-NNN is added to requirements.md. The `/sdd update` command marks design.md, tasks.md, and tests.md as "needs review". The developer runs `/sdd design --merge` to add a PROP-NNN for the new requirement without overwriting existing properties.
...
**Scenario 3 — Security requirement added**: A new SEC-REQ-* is added (e.g., because SECURITY_UNIVERSAL.md was updated with a new threat category). Running `/sdd requirements --security-refresh` re-injects all SEC-REQ-* entries and marks design.md and tests.md as "needs review" for the security sections.

**Scenario 4 — Full regeneration**: The developer runs `/sdd requirements && /sdd design && /sdd tasks && /sdd tests` in sequence. All artifacts are regenerated from scratch. This is appropriate when input documents have changed significantly. Hand-edited content will be lost unless `--merge` is used.
...
sdd/                          (framework installation, not per-project)
  framework/
    SPEC.md                   <- this file
    levels/
  ...
  commands/
    init.md                   <- /sdd init command spec
    requirements.md           <- /sdd requirements command spec
    design.md                 <- /sdd design command spec
    tasks.md                  <- /sdd tasks command spec
    tests.md                  <- /sdd tests command spec
    validate.md               <- /sdd validate command spec
    update.md                 <- /sdd update command spec
    status.md                 <- /sdd status command spec
  agents/
...
| Term               | Definition                                                                                       |
|--------------------|--------------------------------------------------------------------------------------------------|
| Artifact           | One of the four SDD documents: requirements.md, design.md, tasks.md, tests.md                  |
